...
api_expose('newpic');
function newpic(){
    if(!empty($_POST['qrlink'])){
         $value = $_POST['qrlink'];
    }else{
        return ;
    }
    $logo = $_POST['qrico'];
    $qrlength = $_POST['qrlength'] ? $_POST['qrlength'] : 6;
    include THIS_TEMPLATE_DIR.'../../phpqrcode.php';  
    ...
    //输出图片 
    $qrcode_name = time().mt_rand(1,100000).'.png';
    imagepng($QR, THIS_TEMPLATE_DIR.'../../cache/thumbnails/qrcode/'.$qrcode_name); 

    return site_url().'userfiles/cache/thumbnails/qrcode/'.$qrcode_name;
...
function is_manager()
{
    return mw()->user_manager->is_manager();
}
